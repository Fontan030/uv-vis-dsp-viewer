...
def select_language(lang_code):
    ...
    meta_inf_msg.config(text=restart_alert)
...
def show_settings():
    ...
    uv_vis_label_1.grid(row=0, column=0, columnspan=3, sticky='w', padx = 10)
    ...
    uv_label_1 = tk.Label(settings_window, text=_('UV range'))
    ...
    vis_label_1 = tk.Label(settings_window, text=_('Visible range'))
    ...
    reset_btn = tk.Button(settings_window, text=_('Reset'), command=lambda: set_config('default', True) )
...
def show_about():
    ...
    for r in range(4): about_window.rowconfigure(index=r, weight=1)
    tk.Label(about_window, text=f'{app_name} {app_version}').grid(row=0, sticky='ew')
    about_text_1 = _('A viewer for DSP spectral data files recorded by VISIONlite Scan')
    about_msg_1 = tk.Message(about_window, text=about_text_1, justify='center', width=288).grid(row=1)
    about_text_2 = _('Powered by matplotlib and tkinter')
    about_msg_2 = tk.Message(about_window, text=about_text_2, justify='center', width=288).grid(row=2)
